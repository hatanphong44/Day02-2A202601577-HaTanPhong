...
| **Non-AI alternative đã thử chưa?**         | - Thay đổi rubric chấm điểm (chất lượng > số lượng) — cần quy trình duyệt, chậm. - Giới hạn bài/ngày — giảm volume nhưng không giải quyết bài farm vẫn lẫn vào. - Reaction/upvote từ học viên — passive, không ai bấm đều, mentor vẫn phải đọc hết để chấm.           |
...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------- |
| **Rule**     | 1. Thay đổi rubric: chấm chất lượng, không chấm số lượng 2. Giới hạn 1 bài/ngày/người 3. Upvote/reaction từ học viên làm signal lọc                                                                                                                  | Thay đổi quy tắc chấm điểm / cấu hình channel           | - Chi phí 0, triển khai ngay - Giải quyết root cause (incentive)                                         | - Cần consensus giảng viên/mentor - Upvote passive, không đáng tin cậy - Không giảm workload mentor ngay lập tức  | ⭐⭐⭐⭐⭐   |
| **Workflow** | AI Daily Digest: 1. Mỗi sáng AI quét kênh → phân loại bài (farm/chất lượng) 2. Chấm điểm sơ bộ / ranking 3. Gửi digest: Top 5 bài hay + summary cho học viên 4. Mentor dashboard: flag bài farm, ưu tiên chấm bài quality                            | Human-in-the-loop: AI pre-process → Human review/decide | - Giữ nguyên rubric hiện tại - Giảm thời gian cả 2 phía ngay - Có fallback rõ ràng - Dữ liệu text sẵn có | - Cần prompt engineering cho phân loại/chấm điểm - False positive/negative cần monitor                            | ⭐⭐⭐⭐⭐   |
| **Agent**    | Discord Bot tự động: 1. Listen event message mới 2. Phân loại real-time → gán label/thread 3. Tự reply reminder nếu bài quá ngắn/không có code/ví dụ 4. Tự tổng hợp digest hàng ngày post vào channel #daily-digest 5. Mentor chỉ xem channel digest | Fully automated loop: trigger → act → respond           | - Real-time, không chờ sáng hôm sau - Có thể can thiệp ngay lúc post (nudge quality)                     | - Rủi ro cao: bot reply sai → spam/annoy - Cần quyền bot, infra chạy 24/7 - Overkill cho bài toán "lọc & tóm tắt" | ⭐⭐☆☆☆   |
...
## CURRENT STATE — Học viên: ~30 phút/sáng | Mentor: ~30-55 phút/ngày (tỷ lệ với số bài)
...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| **No AI / Process Fix** | Không dùng AI: chỉ đổi rubric, bỏ khuyến khích số lượng, mentor vẫn đọc/chấm thủ công toàn bộ bài | Đủ nếu số bài giảm mạnh và mentor vẫn đọc hết được trong thời gian chấp nhận được | Không xử lý backlog hiện tại, học viên vẫn khó tìm bài hay, không có digest/priority queue | **Không chọn làm chính** |
...
|---|---|---|
| Actor và workflow đã rõ chưa? | **Yes** | Actor gồm học viên cần đọc bài hữu ích và mentor/TA cần lọc/chấm bài. Workflow hiện tại và tương lai đều đã mô tả rõ: Discord gốc → đọc từng bài → đánh giá/chấm; tương lai là AI Daily Digest + Mentor Pre-screen. |
| Baseline và success metric đã đo được chưa? | **Not Yet** | Đã có baseline ước lượng: học viên mất khoảng 30 phút/sáng, mentor mất khoảng 30-55 phút/ngày tùy số bài. Tuy nhiên cần đo lại bằng log thật trong 1 tuần để chốt số bài/ngày, thời gian mentor lọc/chấm và tỷ lệ bài farm/quality. |
| Có data/input đủ dùng chưa? | **Not Yet** | Có nguồn dữ liệu khả thi là lịch sử message Discord, attachment, reaction, thread và điểm mentor đã chấm. Nhưng trước pilot cần export 4-6 tuần dữ liệu và gán nhãn một tập mẫu farm/quality làm ground truth. |
| Nếu AI sai, hậu quả có chấp nhận được không? | **Yes** | Chấp nhận được nếu AI chỉ pre-score, phân loại, tóm tắt và ranking. AI không ghi điểm chính thức, không xóa bài, không reply tự động. Mentor vẫn review/override trước khi dùng để chấm điểm. |
| Có người review/owner vận hành không? | **Yes** | Mentor/TA là người review cuối, override nhãn sai và dùng feedback làm ground truth để cải thiện prompt. Học viên vẫn tự quyết định mở đọc bài nào từ digest. |
| Có cách non-AI đơn giản hơn không? | **Yes** | Có thể đổi rubric, giới hạn 1 bài/ngày/người hoặc dùng upvote/reaction. Tuy nhiên các cách này không giảm workload mentor ngay lập tức và không tạo digest chủ động cho học viên. |
...
```text
Bài toán có actor rõ, workflow rõ, bottleneck nằm ở bước đọc và đánh giá từng bài Discord. Giải pháp phù hợp nhất là Workflow, không phải Agent: AI chỉ hỗ trợ lọc sơ bộ, gợi ý nhãn, pre-score và tạo digest; mentor/TA vẫn là người chấm điểm chính thức và override khi AI sai.

Không chọn No AI / Rule làm hướng chính vì chỉ đổi rubric hoặc giới hạn số bài không giải quyết ngay việc học viên phải tự scroll và mentor vẫn phải đọc nhiều bài. Không chọn Agent vì bot real-time tự reply/gán nhãn có rủi ro UX cao, cần hạ tầng vận hành 24/7 và không cần thiết cho bài toán lọc/tóm tắt hàng ngày.
```

Nếu Go, pilot nhỏ nhất là:

```text
Pilot 1 tuần trên một kênh Discord chia sẻ:

1. Export message Discord của 4-6 tuần gần nhất và điểm mentor đã chấm nếu có.
2. Chọn khoảng 100-200 bài, mentor/TA gán nhãn mẫu: Farm / Quality / Unsure.
3. Viết prompt few-shot dựa trên rubric và ví dụ đã gán nhãn.
4. Mỗi sáng tạo Daily Digest gồm Top 5 bài đáng đọc + tóm tắt 1 câu.
5. Tạo bảng pre-screen đơn giản cho mentor: bài Quality, Farm, Unsure, điểm sơ bộ và lý do.
6. Mentor override nhãn sai, ghi lại feedback để cải thiện prompt.
7. Đo các metric: thời gian học viên đọc digest, thời gian mentor lọc/chấm, digest recall, false negative rate, false positive rate.
```

Nếu Not Yet, cần validate gì trước:

```text
Trước khi mở rộng sau pilot, cần validate:

1. Số bài Discord trung bình mỗi ngày và tỷ lệ bài farm/quality thật.
2. Thời gian mentor/TA đang mất để đọc, lọc và chấm bài.
3. Tiêu chí phân biệt Farm / Quality / Unsure có đủ rõ và mentor có đồng thuận không.
4. AI có đạt ngưỡng chấp nhận được không: F1 > 0.85 cho phân loại farm/quality, false negative < 5%, digest recall ≥ 80%.
5. Học viên có thật sự dùng digest thay vì vẫn phải scroll kênh gốc không.
```

Nếu No-Go, nên làm gì thay AI:

```text
Nếu pilot không đạt, quay về hướng No AI / Rule:

1. Đổi rubric: ưu tiên chất lượng hơn số lượng bài post.
2. Giới hạn số bài được tính điểm mỗi ngày/mỗi tuần.
3. Bắt buộc bài chia sẻ có template tối thiểu: vấn đề, cách làm, ví dụ/code/link, bài học rút ra.
4. Dùng reaction/upvote làm tín hiệu phụ, không dùng làm điểm chính.
5. Mentor chỉ chấm các bài đạt checklist tối thiểu, các bài quá ngắn/trùng/repost không tính điểm.
```
